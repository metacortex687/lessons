...
# тесты → ./task8_test.py: https://github.com/metacortex687/lessons/blob/main/Recursion/task8_test.py
...
def find_files(path: str, _lst: list[str] | None = None) -> list[str]:
    if _lst is None:
        _lst = []
    ...
    for entry in entries:
        if entry.is_dir():
            find_files(entry.path,_lst)
            continue
        _lst.append(entry.name)
