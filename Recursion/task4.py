...
# Алгоритмическая сложность O(n), где n - длинна строки
# тесты → ./task4_test.py: https://github.com/metacortex687/lessons/blob/main/Recursion/task4_test.py

# Описание решения:
# Первоначальный вариант использования срезов вида world[1:-1], не подошел так как для стандартного массива python делается поврехностная копия.
# Срез без копирования делается, например при работе с массивами библиотеки NumPy.
#
# Буду использовать индексы. И две функции.
# Пустую строку считаю палиндромом так как она соотвествует условию "s == reverse(s)""
